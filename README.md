...
| OpenRouter provider | HTTP API with its own in-process bounded tool loop (not a subprocess): local workspace tools (list/read/search/grep/create/edit/patch/mkdir/git/run_command, gated by the same read-only / workspace-write / danger-full-access profiles) plus Roblox Studio MCP tools over a live per-run client, per-thread conversation persistence with deterministic token compaction, validated model capabilities, one live streaming bubble per assistant turn, usage/cost reporting, and a budget gate before every request |
| NVIDIA NIM provider | Direct NVIDIA API access with secure key storage, coding models, automatic transient-error retries, and Studio screenshot input for vision-capable models |
...
| Rojo build + open | Builds a missing place file and opens it in Studio; an existing saved `.rbxl` is never rebuilt implicitly |
...
| Stuck-run escalation | On by default (`stuck_detection_enabled`, with a per-agent `stuckDetectionDisabled` opt-out), a Claude run that genuinely stalls — no provider event at all for `stuck_idle_seconds` (default 600), or the same Studio MCP tool-call cycle looping `stuck_repetition_cap` times with no file edit and no new console output — pauses itself and asks the operator to continue or stop, reusing the same `waiting_decision`/question-card machinery as the agent's own interactive questions, live and after a reload. An actively streaming run never trips it, however long it takes. **Continue testing** resumes the same CLI session with detection suppressed for that resumed run; free text also resumes it with detection still armed; a distinct **Stop** button cancels the run cleanly, and cancelling any run parked in `waiting_decision` works from the chat and the Activity view |
