...
## 7. The genre vocabulary

These recur across top Roblox titles, and their absence is what reviewers point at
when they call an interface generic:

- **Title as a ribbon, not a bar.** A skewed plate overhanging the panel's corner,
  with two darker tabs rotated the other way beneath its ends to read as folds. A
  single extra rectangle beside the plate reads as a floating box instead.
- **Close button outside the corner**, overlapping the panel edge.
- **Panel tilted one or two degrees**, with its shadow tilted to match.
- **Colour carries meaning on buttons**: green confirms, red cancels or closes,
  amber is premium or currency, blue edits. A button colour that means nothing is
  the same defect as a glow that means nothing.
- **Rarity as a coloured plate with the tier written in it**, not colour alone —
  a percentage of players cannot separate the hues.
- **A pressed lip**: a darker band along the bottom edge, the face sitting on top.
- **Halftone dot fields and radial sunburst rays** behind featured content. These
  are image textures; without an upload the nearest native substitutes are a
  rotating `UIGradient` and a tinted overlay, which is weaker and worth naming as
  a gap rather than pretending otherwise.
