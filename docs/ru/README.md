...
> **Публичная бета-версия.** Актуальную сборку смотрите на [странице релизов GitHub](https://github.com/10kkyvl/studioforge/releases). Часть описанных ниже функций реализована в коде, но пока не доступна из работающего приложения — такие места отмечены явно. Полную verification-матрицу см. в [Known Limitations](../KNOWN_LIMITATIONS.md).
...
StudioForge получает список инструментов через capability discovery. Доступ к Studio выдаётся run'у только когда открыт ровно один экземпляр Studio: Claude Code использует собственный MCP-клиент, поэтому StudioForge не может закрепить instance на чужом соединении, а официальный launcher не принимает аргумент выбора instance. При нескольких открытых Studio доступ не выдаётся, а run продолжается без Studio. Экран **Сессии Studio** обнаруживает реально открытые instances Studio и позволяет вручную привязать один из них к проекту; обнаружение запускается кнопкой **Обновить** (`POST /api/v1/studio/sessions/refresh`), а не автоматическим фоновым опросом — каждая проверка запускает процесс launcher, который конкурирует с работающим агентом за единственный WS-слот Studio. Для каждого instance показывается его play state. В режиме `--mock` кнопка **Обновить** ничего не делает, а demo-строки не затрагиваются. Изменяющие/playtest jobs держат эксклюзивный resource `studio:<id>`.
...
Установите Rojo 7 CLI и Studio plugin. Проверьте `rojo --version`. Для выбранного `*.project.json` StudioForge собирает (build) place-файл и открывает его в Studio. Live-sync тоже подключён: `POST`/`DELETE /api/v1/projects/{id}/sync` запускают и останавливают сессию `rojo serve <file> --port <unique-port>` — supervised-подпроцесс с потоковым stdout/stderr, запретом duplicate session и возможностью stop/restart; статус, порт и последние логи видны в разделе Overview, а запустить/остановить sync можно и из чата. Для этого нужен `default.project.json` в корне проекта, и live-sync доставляет файлы только в уже открытую Studio — плагину Rojo всё равно нужно один раз за сессию нажать Connect внутри Studio. VS Code extension не заменяет CLI.

## Проекты, команды и конкурентность

Project source остаётся в пользовательском каталоге; приложение хранит canonical path/fingerprint. Новый проект сразу получает default agent; при старте такой агент также добавляется старым проектам, у которых команды не было. В разделе **Команда** можно создавать, редактировать, включать/выключать и запускать агентов Claude Code, OpenRouter и mock. Опциональная `.agent/` может содержать `constitution.yaml` и `requirements.md` — StudioForge читает ровно эти два файла целиком и подставляет их в system prompt каждого run'а. Память проекта управляется в Overview: записи можно просматривать, редактировать, закреплять, удалять и очищать; каждая связана с исходным run, а последний run помечает внедрённые записи. Большие transcripts находятся в SQLite.
...
Pause/resume выполняются между событиями; cancel завершает provider/process tree. После аварии активные runs становятся `interrupted` и доступны для restart. Все histories, tasks, agents, budgets и memory изолированы по `project_id`. Project-scoped memory store на SQLite FTS5 пишет одну запись на каждый завершённый run, подставляет до пяти релевантных прошлых записей в system prompt следующего run'а и даёт кураторский интерфейс в Overview; это по-прежнему память из prompt'ов, а не суммаризация решений агента.

## Безопасность и permissions

- Оставляйте loopback listener; `--unsafe-host` не превращает daemon в безопасный remote service.
- **В продукте нет общего гейта одобрения оператором перед опасным действием** — тип записи `Decision`, endpoint resolve (`POST /api/v1/decisions/{id}/resolve`) и экран review привязаны ровно к одному producer'у: исчерпанному бюджету коррекций playtest; ничто не приостанавливает run перед произвольной правкой файла, деструктивной командой или публикацией. Функция interactive-question (`studioforge-question`) закрывает случай, когда агент останавливается посреди run'а за вводом оператора, а не является общим гейтом одобрения перед деструктивным действием.
- **Git rollback и tag подключены:** `POST /api/v1/runs/{id}/rollback` (`internal/gitops.SafeRollback` — откат на branch `studioforge/rollback-<timestamp>` на проверенном commit, без force-reset и без удаления untracked files) и `POST /api/v1/projects/{id}/git/tag` (`internal/gitops.Tag`) реализованы целиком, end-to-end, с диалогом подтверждения в экране чата. Rollback требует сохранённого checkpoint для этого run'а и отказывает, пока run удерживает write lease проекта. `Status` и `DiffHead` тоже подключены — экран чата показывает диф завершённого run'а относительно его checkpoint commit. (StudioForge отдельно делает auto-commit Git checkpoint перед каждым не-plan run Claude — именно этот checkpoint использует rollback и панель diff.)
...
- **Studio ambiguous:** доступ к Studio выдаётся только при одном открытом instance — закройте лишние окна Studio, оставьте один и повторите попытку. (Используйте **Обновить** в разделе Сессии Studio, чтобы заново просканировать реальные instances, и **Привязать проект**, чтобы прикрепить один вручную; обнаружение не автоматическое.)
