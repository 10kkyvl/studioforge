...
Каталог моделей загружается вживую из публичного Models API OpenRouter и кешируется на 6 часов, с ручным обновлением из Settings. Если запрос к API не проходит, StudioForge использует последний успешный кеш, а если кеша ещё не было — встроенный датированный fallback-снимок. Агентам предлагаются только модели с поддержкой tool calls (поддержка инструментов зависит от конкретной модели и есть не у всех). Кураторская подборка группирует модели по категориям: Free automatic — автоматический бесплатный выбор (`openrouter/free`), Free recommended — рекомендованные бесплатные, Best coding — лучшие для кода, Balanced — сбалансированные, Fast and cheap — быстрые и дешёвые, Strong reasoning — сильное рассуждение, Large context — большой контекст; сверено 2026-07-21.
...
История разговора хранится по chat-треду в SQLite, а не только в памяти процесса, и потому переживает перезапуск daemon; при разрастании истории StudioForge детерминированно сжимает её по числу токенов, не обращаясь к модели за суммаризацией. Run сообщает usage и денежную стоимость (по данным самого OpenRouter либо, если OpenRouter вернул нулевую стоимость, по оценке из прайса каталога), а бюджетные лимиты перепроверяются на каждом шаге tool loop, а не один раз в начале run'а. На моделях со зрением к сообщению можно приложить изображение; на модели без поддержки зрения run завершится понятной ошибкой вместо того, чтобы молча отбросить вложение. После run'а доступен тот же цикл пост-ран playtest-валидации в Studio, что и для Claude Code.
